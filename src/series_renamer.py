...
class SeriesRenamer:
    ...
    def run(self) -> None:
        ...
        episode_renames = self._plan_episode_renames()
        season_renames  = self._plan_season_renames()

        s, f = self._execute_renames(episode_renames + season_renames)
        skipped.extend(s)
        failed.extend(f)
    ...
    def _plan_undo_renames(self) -> list[models.Rename]:
        ...
        for entry in entries:
            if not entry.old_path.exists():
                print(f"[WARNING] Destination '{entry.old_path} already exists\n")
                continue
    ...
    def _execute_renames(self, renames: list[models.Rename]) -> tuple[list[str], list[str]]:
        ...
        for rename in renames:
            if not rename.needs_rename:
                print(f"[WARNING] No rename needed at '{rename.old_path}'\n")
    ...
    def _execute_undo_renames(self, undo_renames: list[models.Rename]) -> tuple[list[str], list[str]]:
        ...
        for undo in reversed(undo_renames):
            if not undo.needs_rename:
                print(f"[WARNING] No rename needed at {undo.old_path}\n")
